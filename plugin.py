...
except ImportError:  # QAction lived in QtWidgets before Qt 6.
    from qgis.PyQt import QtWidgets

    QAction = getattr(QtWidgets, "QAction")


def _enum_value(container, enum_name, member_name):
    """Return a scoped enum with a fallback for older QGIS releases."""
    enum_group = getattr(container, enum_name, None)
    if enum_group is not None:
        return getattr(enum_group, member_name)
    return getattr(container, member_name)


FRAME_STYLED_PANEL = _enum_value(QFrame, "Shape", "StyledPanel")
FRAME_NO_FRAME = _enum_value(QFrame, "Shape", "NoFrame")
...
_processing_flags = getattr(Qgis, "ProcessingAlgorithmFlag", None)
NO_THREADING = (
    getattr(_processing_flags, "NoThreading")
    if _processing_flags is not None
    else getattr(QgsProcessingAlgorithm, "FlagNoThreading")
)
...
TEMPLATE_CSV = (
    "Palette,Tags,Color1,Color2,Color3,Color4,Color5,Color6,"
...
class RampCard(QFrame):
    def __init__(self, plugin, index, name="", colors="", tags=""):
        ...
        self.setSizePolicy(SIZE_POLICY_EXPANDING, SIZE_POLICY_MAXIMUM)
        ...
        layout.addWidget(
            QLabel(
                "Enter Hex Color Codes (comma-separated, #RGB, #RGBA, "
                "#RRGGBB, #RRGGBBAA):"
            )
        )
        ...
        self.gpl_button.clicked.connect(
            lambda: self.plugin.generate_gpl_for_card(self)
        )
        ...
        self.delete_button.clicked.connect(
            lambda: self.plugin.delete_ramp(self)
        )
    ...
    def update_preview(self):
        ...
        tokens = [
            token.strip()
            for token in self.color_input.text().split(",")
            if token.strip()
        ]
        ...
        self.error_label.setText(
            "Invalid hex code(s): {0}".format(", ".join(invalid))
            if invalid
            else ""
        )
    ...
    def get_ramp_data(self):
        ...
        if invalid_colors:
            self.error_label.setText(
                "Invalid hex code(s): {0}. Please fix them.".format(
                    ", ".join(invalid_colors)
                )
            )
            return None
        ...
        return {
            ...
            "ramp_name": self.name_input.text().strip()
            or "UnnamedRamp{0}".format(self.index),
...
class QGISColorRampGenerator(QMainWindow):
    ...
    def _build_ui(self):
        ...
        subtitle = QLabel(
            "Create multiple QGIS preset ramps, preview RGBA colors, import "
            "CSV palettes, and export merged XML or per-ramp GPL files."
        )
        ...
        self.scroll_area.setFrameShape(FRAME_NO_FRAME)
        ...
        self.empty_state = QLabel(
            'Click "Add New Ramp Manually" or "Import CSV" to begin creating '
            "your color palettes."
        )
    ...
    def add_ramp(self, name="", colors="", tags=""):
        if len(self.ramp_cards) >= 100:
            QMessageBox.warning(
                self, "Limit Reached", "Maximum 100 ramps allowed."
            )
            return

        self.ramp_counter += 1
        card = RampCard(
            self, self.ramp_counter, name=name, colors=colors, tags=tags
        )
    ...
    def import_csv(self):
        ...
        if len(header) < 2:
            QMessageBox.warning(
                self,
                "Import Failed",
                "CSV must contain at least a name column and one color "
                "column.",
            )
            return
        ...
        for row in rows[1:]:
            ...
            tags = (
                row[tags_index].strip()
                if tags_index != -1 and tags_index < len(row)
                else ""
            )
            colors = []
        ...
        for card in self.ramp_cards:
            ...
            if not data:
                QMessageBox.warning(
                    self,
                    "Cannot Generate XML",
                    "One or more ramps have invalid colors. Please fix them "
                    "and try again.",
                )
                return
    ...
    def _save_text_file(self, title, suggested_name, filter_text, content):
        file_path, _ = QFileDialog.getSaveFileName(
            self, title, suggested_name, filter_text
        )
        if not file_path:
            return
        try:
    ...
    def _build_xml(self, ramp_entries):
        ...
        for entry in ramp_entries:
            ...
            for index, color_data in enumerate(entry["valid_colors"]):
                ...
                lines.append(
                    (
                        '    <Option type="QString" name="preset_color_{0}" '
                        'value="{1},{2},{3},{4}"/>'
    ...
    def _build_gpl_filename(self, ramp_name):
        ...
        return (
            "{0}_{1}.gpl".format(prefix, safe_ramp)
            if prefix
            else "{0}.gpl".format(safe_ramp)
        )
